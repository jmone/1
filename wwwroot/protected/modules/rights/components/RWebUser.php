...
class RWebUser extends CWebUser
{
	...
	*/
	...
	{
		//解析controller嵌套的文件夹 并授予权限 Code By RuiZi ----Begin
		if(strpos($operation,'/') ==true){
			$Tmpa = explode('/',$operation);
			$Tmpb = explode('.',$Tmpa[0]);
			$operation = $Tmpb[0].'.'.ucfirst($Tmpa[1]);
		}
		//解析controller嵌套的文件夹 并授予权限 Code By RuiZi ----End
